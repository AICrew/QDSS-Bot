...
module.exports = {
  ...
  */
  open : function ()
  {
    ...
    /** Queries the database for the existence of a user, and proceeds to create
      * the corresponding entry if not present already
    ...
    database.insertUserIfNotExists = function (userId, username)
    {
      return new Promise( (resolve, reject) => {
        this.getAsync("SELECT * FROM Utenti WHERE userId = ?", [userId])
          .then( (user) => { if (!user) return this.runAsync("INSERT INTO Utenti (userId, username) VALUES(?,?)", [userId, username]) })
          .then( () => resolve() )
