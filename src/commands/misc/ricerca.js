...
class Cmd_Ricerca extends Command {
  constructor(client) {
    super(client, {
      name: "ricerca",
  ...
  async run(message, args) 
  {
    ...
    switch (target)
    {
      ...
      default:
        message.reply("Il primo parametro del comando deve essere uno tra i seguenti: " +
  ...
  search(keywords, options, function(err, results)
  {
    ...
    for (let i = 0; i < 3; i++)
    {
      ...
      else if (i == 0)	// Se la ricerca non ha prodotto risultati
      {
        ...
        responseChannel.send({ embeds: [noResults] });
      }
    }
  });
}
...
function wordpressSearch(keywords, embedColor, responseChannel)
{
  ...
  responseChannel.send(msg).then(async function(msg)
  {
