  ...
  async run(message, args, level) 
  {
	...
	switch (actionCmd)
	{
		...
		default:
			...
			message.reply("il parametro `" + actionCmd + "` non è un'azione valida per il comando +gamedb");
		return;
	}
	...
	if (action == ACTION_ADD_GAME_DB)
	{
		...
		db.getAsync("SELECT * FROM Giochi WHERE nome = ?", [game]).then( async (gameExists) => 
		{
			if (!gameExists)
			{
				...
				await db.runAsync(query_insert).then( () => 
				{
			...
			else message.channel.send(gameName + " è già presente nel database.");
		})
		.then( () => db.close() )
		.catch( (err) => {
			db.close();
	...
	else if (action == ACTION_REMOVE_GAME_DB)
	{
		...
		if (args.length == 0)
		{
			message.reply("il corretto formato del comando è `+gamedb remove <nome-gioco>`");
			return;
		}
		...
		db.getAsync("SELECT * FROM Giochi WHERE nome = ?", [game]).then( async (row) => 
		{
			...
			await db.runAsync("DELETE FROM Registrazioni WHERE game = ?", [game]).then( () => 
			{
			...
			await db.runAsync("DELETE FROM Giochi WHERE nome = ?", [game]).then( () => 
			{
			...
			if (role)
			{
				role.delete("Eliminazione del gioco " + row.nomeCompleto + " dal sistema")
					.then(role => message.channel.send("Il ruolo [" + row.nomeCompleto + "] è stato eliminato con successo dal server :white_check_mark:"))
					.catch(console.error);
			}
			...
			{
				channel.delete()
					.then(chan => message.channel.send("La stanza [" + chan.name + "] è stata eliminata con successo dal server :white_check_mark:"))
					.catch(console.error);
			}
	...
	else if (ACTION_EDIT_COLOR_GAME_DB)
	{
		// ==========================================================
		// =														=
		...
		if (!game || !color)
		{
			message.reply("formato del comando errato, usare `+gamedb color <nome-gioco> 0xCOLORE`");
			return;
		}
		...
		db.getAsync("SELECT * FROM Giochi WHERE nome = ?", [game]).then( async (row) => 
		{
			...
			{                            // Modifica del colore associato al ruolo del gioco
				role.setColor(color)
					.then(role => message.channel.send("Il ruolo [" + row.nomeCompleto + "] è stato aggiornato con il colore " + color + " :white_check_mark:"))
					.catch(console.error);
			}
			else message.channel.send("Impossibile aggiornare il colore del ruolo [" + row.nomeCompleto + "] :x:");
		})
		.then( () => db.close() )
		.catch( (err) => {
			db.close();
